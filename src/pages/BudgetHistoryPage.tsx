...
import { ROUTES, budgetHistoryChartsPath, budgetHistoryDetailPath } from '../config/routes';
        ...
        <ul className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 divide-y divide-slate-200 dark:divide-slate-700">
          {archivedSubBudgets.map((budget) => (
            <li key={budget.id} className="flex items-stretch">
              ...
              <Link
                to={budgetHistoryChartsPath(budget.id)}
                className={`flex items-center justify-center px-3 min-h-[56px] min-w-[44px] text-sm font-medium shrink-0 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors ${SEMANTIC_COLORS.interactive.text}`}
              >
                {t('nav.charts')}
              </Link>
              <button
