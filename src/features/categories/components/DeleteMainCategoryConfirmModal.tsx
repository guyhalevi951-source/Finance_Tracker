...
interface DeleteMainCategoryConfirmModalProps {
  open: boolean;
  categoryName: string;
  isSaving: boolean;
  onConfirm: () => void;
...
export function DeleteMainCategoryConfirmModal({
  open,
  categoryName,
  isSaving,
  onConfirm,
  onDismiss,
}: DeleteMainCategoryConfirmModalProps) {
      ...
      >
        <div className="flex items-start justify-between mb-4">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">
            {t('category.management.deleteConfirmTitle')}
          </h3>
          <button
            type="button"
            onClick={onDismiss}
            ...
            aria-label={t('category.management.deleteConfirmCancel')}
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        <p className="text-slate-600 dark:text-slate-300 mb-6">
          {t('category.management.deleteConfirmMessage', { name: categoryName })}
        </p>
        <div className="flex gap-3">
          ...
          >
            {t('category.management.deleteConfirmCancel')}
          </button>
          <button
            type="button"
            onClick={onConfirm}
          ...
          >
            {t('category.management.deleteConfirmConfirm')}
          </button>
        </div>
      </div>
    </div>
