...
export function useCategories(userId: string | null): UseCategoriesReturn {
  ...
  const addCustomCategory = useCallback(
    ...
    [userId, mainCategories, subCategories],
  );
  ...
  const addMainCategory = useCallback(
    async (input: MainCategoryInput, locale: AppLocale): Promise<MainCategoryRecord | null> => {
      ...
      try {
        ...
        setMainCategories((prev) => sortMainCategories([...prev, main]));
  ...
  const updateMainCategory = useCallback(
    async (
      ...
      try {
        ...
        await saveSubCategories(userId, updatedSubs.filter((s) => s.parentId === mainId));
        setMainCategories((prev) =>
          sortMainCategories(prev.map((m) => (m.id === mainId ? updatedMain : m))),
        );
        setSubCategories((prev) =>
          prev.map((sub) => updatedSubs.find((u) => u.id === sub.id) ?? sub),
        );
        return updatedMain;
      } catch {
        setMainCategoryActionError('saveFailed');
  ...
  const deleteMainCategoryAction = useCallback(
    async (mainId: string): Promise<boolean> => {
      ...
      try {
        ...
        setMainCategories(sortMainCategories(result.mains));
        setSubCategories(result.subs);
        return true;
      } catch {
