...
interface DeleteScheduledExpenseConfirmModalProps {
  open: boolean;
  target: Expense | null;
  locale: AppLocale;
  isSaving: boolean;
...
export function DeleteScheduledExpenseConfirmModal({
  open,
  target,
  locale,
  isSaving,
  onConfirm,
  onDismiss,
}: DeleteScheduledExpenseConfirmModalProps) {
  ...
  const label = hasDescription ? descriptionText : t('profile.settings.oneTime.unnamed');
      ...
      >
        <div className="flex items-start justify-between mb-2">
          <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-100">
            {t('profile.settings.oneTime.deleteTitle')}
          </h3>
          <button
            type="button"
            onClick={onDismiss}
            ...
            aria-label={t('profile.settings.oneTime.deleteCancel')}
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <p className="text-slate-600 dark:text-slate-300 mb-2">
          {t('profile.settings.oneTime.deleteMessage')}
        </p>
        ...
        <div className="flex flex-col gap-3">
          ...
          >
            {t('profile.settings.oneTime.deleteCancel')}
          </button>
          <button
            type="button"
            onClick={onConfirm}
          ...
          >
            {t('profile.settings.oneTime.deleteConfirm')}
          </button>
        </div>
      </div>
    </div>
