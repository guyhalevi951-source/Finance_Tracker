...
import { type MonthBudgetController } from '../hooks/useMonthBudgetController';
...
/** Shared native checkbox styling for budget forms and rows. */
export const BUDGET_CHECKBOX_CLASS =
...
export interface MasterBudgetListRowProps {
  ...
  >;
  onOpenOverview: () => void;
  /** Row title; defaults to the master monthly budget title. Fixed budgets pass their name. */
  title?: string;
  /** Extra actions (edit/delete/reorder) rendered above the amount controls. */
...
export function MasterBudgetListRow({
  ...
  const isMasterRow = title === undefined;
  ...
  const carryoverCheckboxClass = isEnglish ? BUDGET_CHECKBOX_CLASS : `${BUDGET_CHECKBOX_CLASS} mt-0.5`;

  return (
    <div
      className={`grid grid-cols-[minmax(0,1fr)_auto] sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] items-start gap-x-3 gap-y-2 ${BUDGET_LIST_ROW_LAYOUT} rounded-lg bg-slate-50 dark:bg-slate-900/40`}
    >
      <div className="min-w-0 order-1">
        <p className="font-medium text-slate-800 dark:text-slate-100 truncate">
          {title ?? t('budget.monthlyBudgetTitle')}
        </p>
        ...
        {isMasterRow && <span className="sr-only">{t('budget.list.masterLocked')}</span>}
      </div>

      {/* Month navigator: full-width row on narrow screens, center column from sm up. */}
      <div className="order-3 col-span-2 justify-self-center self-start shrink-0 sm:order-2 sm:col-span-1">
      ...
      <div
        className={`order-2 sm:order-3 justify-self-end flex flex-col items-stretch gap-1 shrink-0 ${BUDGET_ACTION_CLUSTER_WIDTH_CLASS}`}
      >
        {trailingActions}
